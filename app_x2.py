...
class LoginManager:
    ...
    def _load_users(self):
        ...
        self.users_file = Path("users.pkl")
        if self.users_file.exists():
            with open(self.users_file, "rb") as f:
                self.users = pickle.load(f)
        else:
            # Create default admin user - in production, use environment variables
    ...
    def _verify_password(self, stored_hash: str, provided_password: str) -> bool:
        ...
        return hmac.compare_digest(
            stored_hash,
            self._hash_password(provided_password)
        )
    ...
    def _handle_failed_attempt(self):
        ...
        if st.session_state.login_attempts >= self.max_attempts:
            st.session_state.locked_until = datetime.now() + timedelta(seconds=self.lockout_duration)
            st.error(f"Too many failed attempts. Account locked for {self.lockout_duration // 60} minutes.")
        ...
        with st.container():
            ...
            if st.session_state.get('show_reset', False):
                ...
                if st.button("Submit Reset"):
                    ...
                    elif login_manager.reset_password(username, old_password, new_password):
...
# Modified main function to include login
def main():
