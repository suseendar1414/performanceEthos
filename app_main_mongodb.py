...
def get_mongodb_connection():
    """Create and return a new MongoDB connection instance"""
    try:
        ...
        if not username or not password:
            st.error("MongoDB credentials not found in environment variables.")
            return None, None
        ...
        mongo_uri = f"mongodb+srv://{encoded_username}:{encoded_password}@cluster0.vxqak.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
        ...
        st.error(f"Error creating MongoDB connection: {str(e)}")
        return None, None

def test_mongodb_connection(db, client):
...
def main():
    st.set_page_config(page_title="Ethos Lending Mortgages Under Management (MUM)", layout="wide")
    
    # Load environment variables
    load_dotenv()

    required_env_vars = ['MONGODB_USERNAME', 'MONGODB_PASSWORD']
    ...
    try:
        ...
        # Proper way to check connection
        if not test_mongodb_connection(db, client):
            st.error("Could not establish connection to MongoDB. Please check your credentials.")
            return
        ...
        # Get available years from MongoDB
        try:
            pipeline = [
                {
                    "$match": {
                        "Close Date": {"$exists": True, "$ne": None}
                    }
                },
                {
                    "$group": {
            ...
            available_years = [doc["_id"] for doc in db.loans.aggregate(pipeline)]
            
            if len(available_years) == 0:
                st.error("No data available in MongoDB. Please check your data.")
                return
        ...
        # Rest of your existing tab content code remains the same
        with tab1:
