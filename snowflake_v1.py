...
def display_visualizations(scores, selected_year):
    """Display all visualizations with proper error handling"""
    try:
        ...
        with col1:
            ...
            st.plotly_chart(gauge_fig, use_container_width=True, key=f"gauge_{selected_year}")
            
            # Radar Chart
            st.subheader("KPI Achievement Distribution")
            radar_fig = create_radar_chart(scores, selected_year)
            st.plotly_chart(radar_fig, use_container_width=True, key=f"radar_{selected_year}")
        
        with col2:
            # KPI Scores vs Weights Bar Chart
            st.subheader("KPI Scores and Weights")
            bar_fig = create_bar_chart(scores, selected_year)
            st.plotly_chart(bar_fig, use_container_width=True, key=f"bar_{selected_year}")
            
            # Loan Type Distribution (if available)
            if 'Loan_Types' in scores[selected_year]['detailed_scores']:
                ...
                st.plotly_chart(loan_type_fig, use_container_width=True, key=f"loan_types_{selected_year}")

        # Optional: Add overall trends at the bottom
        st.markdown("---")
        st.subheader("Performance Trends")
        year_trend_fig = create_year_over_year_comparison(scores)
        st.plotly_chart(year_trend_fig, use_container_width=True, key=f"trends_{selected_year}")

        # Add explanatory text
        st.markdown("""
        ### Chart Explanations:
        * **Overall Performance**: Gauge showing the total achievement score for the selected year
        * **KPI Achievement Distribution**: Radar chart showing achievement scores across all KPIs
        * **KPI Scores and Weights**: Comparison of actual scores against assigned weights
        * **Loan Type Distribution**: Breakdown of loan types (if available)
        * **Performance Trends**: Year-over-year performance comparison
        """)

    except Exception as e:
        st.error(f"Error displaying visualizations: {str(e)}")
        st.error("Please check if all required data is available and properly formatted.")
...
# Update the code in the Year Comparison tab
def display_year_comparison(scores, available_years):
    """Display year comparison charts with unique keys"""
    # Year-over-year comparison chart
    yoy_fig = create_year_over_year_comparison(scores)
    st.plotly_chart(yoy_fig, use_container_width=True, key="yoy_comparison_tab")
...
def get_snowflake_connection():
    """Create and return a new SnowflakeConnection instance"""
...
class SnowflakeConnection:
    def __init__(self):
        ...
        try:
            ...
            st.error(f"Error initializing Snowflake connection: {str(e)}")

    def connect(self):
        """Establish connection to Snowflake"""
        try:
            # Print connection parameters for debugging (remove in production)
            st.write("Attempting to connect with following parameters:")
            st.write(f"Warehouse: COMPUTE_WH")
            st.write(f"Database: FIRSTDB")
            ...
            st.error("Please check your account identifier format. It should be in the format: orgname-accountname")
            raise
        except Exception as e:
            st.error(f"Failed to connect to Snowflake: {str(e)}")
            st.error("Please verify your Snowflake credentials and connection parameters.")
            raise
    ...
    def disconnect(self):
        ...
        if self.conn:
            try:
                ...
                st.error(f"Error closing Snowflake connection: {str(e)}")

    def fetch_loan_data(self, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        """
        Fetch loan data from Snowflake table within date range
        """
        try:
            ...
            SELECT EXISTS (
                ...
                WHERE TABLE_NAME = 'DINO_CSV'
            )
            """
            exists_df = pd.read_sql(verify_query, self.conn)
            if not exists_df.iloc[0, 0]:
                st.error("Table 'DINO_CSV' does not exist in the database.")
                return None
            ...
            st.success(f"Successfully fetched {len(df)} records from Snowflake")
            return df
    ...
    def get_available_years(self) -> list:
        ...
        try:
            query = """
            SELECT DISTINCT YEAR(CLOSE_DATE) as year
            FROM DINO_CSV 
            WHERE CLOSE_DATE IS NOT NULL
            ORDER BY year
            """
            
            years_df = pd.read_sql(query, self.conn)
            years = years_df['YEAR'].tolist()
            
            if not years:
                st.warning("No years found in the data, using current year as default")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            st.success(f"Connection test successful! Snowflake version: {version}")
            return True
    ...
    def get_table_schema(self):
        """Get the schema of the DINO_CSV table"""
        try:
            query = """
            ...
            """
            schema_df = pd.read_sql(query, self.conn)
            return schema_df
        except Exception as e:
...
def main():
    ...
    try:
        ...
        if not snow_conn.test_connection():
            st.error("Could not establish connection to Snowflake. Please check your credentials.")
            return
        ...
        # Get available years from Snowflake
        available_years = snow_conn.get_available_years()
        
        if not available_years:
            st.error("No data available in Snowflake. Please check your data.")
            return

        # Fetch data from Snowflake for all years
        with st.spinner("Fetching data from Snowflake..."):
            # Modified to fetch all data without date filters
            query = """
            SELECT 
                CLOSE_DATE as "Close Date",
                CREATED_DATE as "Created Date",
            ...
            WHERE CLOSE_DATE IS NOT NULL
            """
            data = pd.read_sql(query, snow_conn.conn)
        ...
        selected_year = st.sidebar.selectbox(
            ...
            options=sorted(list(scores.keys())),
            index=len(scores.keys())-1
        )
        ...
        with tab3:
            display_visualizations(scores, selected_year)
        ...
        with tab4:
            ...
            if comparison_data:
                ...
                if kpi_comparison_data:
                    kpi_df = pd.DataFrame(kpi_comparison_data)
                    heatmap_fig = go.Figure(data=go.Heatmap(
                        z=kpi_df.pivot(index='Year', columns='KPI', values='Achievement Score'),
                        x=kpi_df['KPI'].unique(),
                        y=kpi_df['Year'].unique(),
                        colorscale='Viridis',
                        text=kpi_df.pivot(index='Year', columns='KPI', values='Achievement Score').round(1),
                        texttemplate='%{text}%',
                        textfont={"size": 10},
                        hoverongaps=False
                    ))
                    ...
                    st.plotly_chart(heatmap_fig, use_container_width=True, key="kpi_heatmap")
            else:
                st.warning("No year-over-year comparison data available.")
        
        with tab5:
            add_ai_insights_tab(scores, selected_year)
        
        # Display any warnings
        if selected_year in scores and scores[selected_year].get('warnings'):
